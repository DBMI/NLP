...
#from numpy import 
#from itemData_UPDATED import *
#import matrix
import InteractItems as inter
...
class tagObject(object):
    ...
    def limitScope(self,obj):
        #print self.getCategory(),obj.getCategory()
        """If self and obj are of the same category or if obj has a rule of
        'terminate', use the span of obj to update the scope of self"""
        if( not self.getRule() or self.getRule()== 'terminate' or 
             (self.getCategory() != obj.getCategory() and obj.getRule() != 'terminate')):
            #print "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
            return
        if (obj.getRule() == 'terminate' and self.getCategory() != obj.getCategory()):
#            print "***************************************************************"
#            print "%s is applied to %s" %(obj.getLiteral(),self.getLiteral())
#            print "%s is applied to %s" %(obj.getCategory(),self.getCategory())
#            print "***************************************************************"
            try:
                checkInteraction = inter.ItemInteractions()
                if checkInteraction(obj.getCategory(),self.getCategory()):
#                    print "***************************************************************"
#                    print "%s is applied to %s" %(obj.getLiteral(),self.getLiteral())
#                    print "%s is applied to %s" %(obj.getCategory(),self.getCategory())
#                    print "***************************************************************"
                    if ('forward' in self.getRule().lower() and (obj>self)):
                        self.__scope[1] = min(self.__scope[1],obj.getSpan()[0])  
                    elif( 'backward' in self.getRule().lower() and obj < self):
                        self.__scope[0] = max(self.__scope[0],obj.getSpan()[1])
##                else:
##                    feedback = raw_input("Create a new interaction pair %s and %s? Y/N") %(self.getCategory(),obj.getCategory())
##                    if feedback.lower() == "y":
##                        checkInteraction.setInteractions(obj.getCategory(),self.getCategory())
##                    else:
##                        pass
            except:
                pass
#                print "%s cannot be applied to %s" %(obj.getLiteral(),self.getLiteral())
#                print "%s cannot be applied to %s" %(obj.getCategory(),self.getCategory())
        else:
            if obj.getRule()!= 'terminate':
                #print "####################################################"
                if ('forward' in self.getRule().lower() and (obj>self)):
                    self.__scope[1] = min(self.__scope[1],obj.getSpan()[0])
                elif( 'backward' in self.getRule().lower() and obj < self):
...
class pyConText(object):
    ...
    def dropMarks(self,category="exclusion"):
        ...
        #print self.__graph.nodes()[1].getCategory()
